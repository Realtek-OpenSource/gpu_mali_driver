...
extern int ump_ops_register(struct ump_operations * cb);
struct ump_operations *ump_ops;
/*
...
static int __init umpp_linux_initialize_module(void)
{
    ...
    //printk(KERN_ALERT "[+] register ops\n");
    //extern struct ump_operations ops;

    if (!ump_ops) {
        ump_ops = kzalloc(sizeof(*ump_ops), GFP_KERNEL);
        if (ump_ops == NULL) {
            printk(KERN_ALERT "can not allocate\n");
            return 0;
        }
    }

    ump_ops->create_from_phys_blocks_64 = ump_dd_create_from_phys_blocks_64;
    ump_ops->create_from_phys_blocks = ump_dd_handle_create_from_phys_blocks;
    ump_ops->secure_id_get = ump_dd_secure_id_get;
    ump_ops_register(ump_ops);
    //printk(KERN_ALERT "[-] register ops\n");
